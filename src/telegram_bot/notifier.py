...
class TelegramNotifier(Notifier):
	def __init__(self, notification_store: NotificationStore, cache: CourtCache, subscriber: CourtSubscriber, bot: Bot):
		super().__init__(notification_store, cache, subscriber)
		self._bot = bot
	...
	async def _send(self, message: str, user_ids: set[str]) -> None:
		for user_id in user_ids:
			try:
				...
				logger.exception(f'Failed to notify user: {message}')
